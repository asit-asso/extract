...
public class LocalizedMessages {
     ...
     * @return the string localized in the current language, or the key if not found
     */
    ...
    /**
     * Obtains the help content in the specified format.
     *
     * @param format the format of the help content (e.g., "html")
     * @return the help content, or null if not available
     */
    public final String getHelp(final String format) {
        if ("html".equalsIgnoreCase(format)) {
            return this.getFileContent("fmeServerV2Help.html");
        }
        return null;
    }
    ...
    private void loadFile(final String guiLanguage) {
        ...
        for (String filePath : this.getFallbackPaths(guiLanguage, LocalizedMessages.MESSAGES_FILE_NAME)) {
            ...
            try (InputStream languageFileStream = this.getClass().getClassLoader().getResourceAsStream(filePath)) {
                ...
                loadedProperties.load(languageFileStream);
                this.logger.debug("Loaded localization file from \"{}\".", filePath);
                break;

            } catch (IOException exception) {
                this.logger.error("Could not load the localization file from \"{}\".", filePath, exception);
        ...
        if (loadedProperties == null) {
            ...
            // Create a minimal fallback properties to prevent null pointer exceptions
            loadedProperties = new Properties();
            // Add some basic fallback messages
            loadedProperties.setProperty("plugin.label", "FME Server V2");
            loadedProperties.setProperty("plugin.description", "FME Server V2 task processor with GeoJSON support");
            loadedProperties.setProperty("errorParameters.noParam", "No parameters provided");
            loadedProperties.setProperty("errorServiceUrl.notDefined", "Service URL not defined");
            loadedProperties.setProperty("errorServiceUrl.invalid", "Invalid service URL");
            loadedProperties.setProperty("errorAuth.notDefined", "Authentication not configured");
            loadedProperties.setProperty("extract.success", "Extraction completed successfully");
            loadedProperties.setProperty("errorDownload.failed", "Download failed");
            loadedProperties.setProperty("errorResponse.noUrl", "No download URL in response");
            loadedProperties.setProperty("errorProcess.failed", "Process failed: %s");
        }

        this.propertyFile = loadedProperties;
        this.logger.info("Localized messages loaded for FME Server V2 plugin.");
    }

    /**
     * Builds a collection of possible paths a localized file to ensure that one is found even if the
